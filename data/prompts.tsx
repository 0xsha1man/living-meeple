// This file has been refactored to support a multi-part prompt structure
// for cost efficiency with the Google AI Files API. The original templates
// are commented out below for comparison.

// --- 1. Identify Common, Reusable Fragments ---
// These are common phrases that can be uploaded to the Files API once.
export const PROMPT_FRAGMENTS = {
  // For storyboard prompts
  USING_CURRENT_MAP: 'Using the current map, ',
  KEEP_UNCHANGED: ' Keep all other elements unchanged.',

  // For map generation prompts
  MAP_STYLE_INSTRUCTIONS: `
  ...
  MAP_DRAWING_INSTRUCTIONS_SUFFIX: ` with color pencil. Use the full image and do not leave gutter or padding, having features flow off the edge of the image. The features MUST be drawn in a generalized, almost iconic way, not a realistic or tactical military map style.
...
// --- 2. Deconstruct Prompt Templates into Prefix, Suffix, and a Dynamic Middle ---
// This structure allows the frontend to construct the dynamic part of the prompt,
// while the backend can use cached file URIs for the static prefix and suffix.
export const DECONSTRUCTED_PROMPTS = {
  map: {
    features: {
      prefix: PROMPT_FRAGMENTS.MAP_STYLE_INSTRUCTIONS + `
      Step 2: Using the provided base image as a canvas, ONLY add features described: "`,
      template: `{{description}}`,
      suffix: `"` + PROMPT_FRAGMENTS.MAP_DRAWING_INSTRUCTIONS_SUFFIX,
    },
    landmarks: {
      prefix: PROMPT_FRAGMENTS.MAP_STYLE_INSTRUCTIONS + `
      Step 2: Using the provided map as a canvas, ONLY add landmarks described: "`,
      template: `{{description}}`,
      suffix: `"` + PROMPT_FRAGMENTS.MAP_DRAWING_INSTRUCTIONS_SUFFIX,
    },
  },
  storyboard: {
    placement: {
      prefix: PROMPT_FRAGMENTS.USING_CURRENT_MAP + 'add ',
      template: '{{amount}} instances of the {{meeple_asset_name}} asset at the location of {{location}}, arranged as {{density}}',
      suffix: '. The placement should be clear and not overcrowd the map. The placement must not also cover other elements already there completely. Shift the placement slightly or layer in such a way that allows both to be seen in whole or part. Scale the assets to fit the image appropriate. The goal is to show a general location, not precise military formation.' + PROMPT_FRAGMENTS.KEEP_UNCHANGED,
    },
    movement: {
      prefix: PROMPT_FRAGMENTS.USING_CURRENT_MAP + 'add in color pencil a simple, clean, wide arrow starting from the area below the meeples at ',
      template: '{{start}}, representing a {{type}} movement, and the arrow tip pointing towards the area of {{end}}',
      suffix: '. The arrow should be the same color as the meeples at the start location. The arrow should clearly indicate direction without cluttering the map. The arrow should be of a low opacity of 20% and blend seemless into the image.' + PROMPT_FRAGMENTS.KEEP_UNCHANGED,
    },
    label: {
      prefix: PROMPT_FRAGMENTS.USING_CURRENT_MAP + 'add the text ',
      template: '"{{text}}" at the following {{location}}',
      suffix: '. Use a clean, bold, arial-type without any glow, stroke or other effects as if written in black color pencil. The label must be legible, placed thoughtfully to not obscure important details, and help in understanding the map, not cluttering it. Double check for spelling.' + PROMPT_FRAGMENTS.KEEP_UNCHANGED,
    },
  },
};

...
const loadStaticAsset = async (url: string, caption: string): Promise<GeneratedAsset> => {
  storyState.addLog(`[Cartographer] Uploading static asset: ${url}`);
...
export const generateBaseAssets = async (
  ...
  storyState.addLog('[Cartographer] Loading neutral map background...');
  ...
  storyState.addLog('[Cartographer] Loading cartography style guide...');
  ...
  for (const map of battlePlan.maps) {
    storyState.addLog(`[Cartographer] Generating map asset: ${map.map_asset_name}`);

    storyState.addLog(`[Cartographer] Adding defining features for ${map.map_asset_name}`);
    ...
    onAssetProgress();
    storyState.addLog(`[Cartographer] Waiting ${IMAGE_GENERATION_DELAY_MS / 1000}s before next step...`);
    await sleep(IMAGE_GENERATION_DELAY_MS);

    storyState.addLog(`[Cartographer] Adding key landmarks for ${map.map_asset_name}`);
  ...
  for (const faction of battlePlan.factions) {
    storyState.addLog(`[Cartographer] Generating meeple asset for: ${faction.name}`);
  ...
  storyState.addLog("[Cartographer] All base assets generated.");
  return assets;
};
...
export const generateStoryboardFrames = async (
  ...
  for (let i = 0; i < battlePlan.storyboard.length; i++) {
    ...
    storyState.addLog(`[Storyteller] Compositing page ${i + 1} of ${battlePlan.storyboard.length}...`);
    ...
    if (frame.placements) {
      for (const placement of frame.placements) {
        storyState.addLog(`[Storyteller] Placing ${placement.amount} ${placement.meeple_asset_name} at ${placement.location}`);
        ...
        storyState.addLog(`[Storyteller] Waiting ${IMAGE_GENERATION_DELAY_MS / 1000}s before next step...`);
        await sleep(IMAGE_GENERATION_DELAY_MS);
      }
    }
    ...
    if (frame.movements) {
      for (const movement of frame.movements) {
        storyState.addLog(`[Storyteller] Adding movement arrows from ${movement.starting_point} to ${movement.end_point}`);
    ...
    if (frame.labels) {
      for (const label of frame.labels) {
        storyState.addLog(`[Storyteller] Adding label "${label.text}" at ${label.location}`);
